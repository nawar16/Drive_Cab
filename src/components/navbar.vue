<template lang="html">
  <nav class="navbar navbar-expand-lg navbar-blue">
    <router-link :to="{ name: 'home', params: {} }" class="navbar-brand bb"><b>QCab</b></router-link>
  ...
  <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
    <div class="navbar-nav">
      ...
      <div v-if="loggedIn" class="">
        <router-link :to="{ name: 'cars', params: {} }" class="nav-item nav-link bb">Cars</router-link>
      </div>
    </div>
...
export default {
  ...
  methods: {
    logout() {
      ...
      this.$store.commit("setOrigin", [34.7324, 36.7137]);
      this.$store.commit("setDestiny", [33.5138, 36.2765]);
